"""Verify the locally built V1 image using a disposable private volume and secret."""
...
def main():
    name = "thryv-v1-check-" + secrets.token_hex(4)
    volume = name + "-data"
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="thryv-container-") as folder:
            ...
            run(["run", "--rm", *common, "thryv-backend:v1", "alembic", "upgrade", "head"])
            print("PASS " + stage)
            stage = "production health, data permissions, schema and artifact exclusion"
            run(["run", "-d", "--name", name, *common, "thryv-backend:v1"])
            script = """
...
with sqlite3.connect('/data/thryv.db') as db:
    assert db.execute('select version_num from alembic_version').fetchone()[0] == '675b403091a8'
try:
